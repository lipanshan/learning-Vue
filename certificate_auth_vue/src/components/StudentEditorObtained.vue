    ...
    <div class="editor-obtained">
        ...
        >
            <el-form-item label="姓名" prop="stu_name">
                <el-input :maxlength="stuNameLen" v-model="form.stu_name" disabled clearable></el-input>
            </el-form-item>
            <el-form-item label="性别" prop="stu_sex">
                <el-radio-group disabled v-model="form.stu_sex">
                    <el-radio :label="1">男</el-radio>
                    <el-radio :label="2">女</el-radio>
                </el-radio-group>
            </el-form-item>
            <el-form-item label="身份证号" prop="id_num">
                <el-input v-model="form.id_num" disabled clearable></el-input>
            </el-form-item>
            ...
            <el-form-item label="证书类型" prop="certificate_id">
                <el-select @change="certificateTypeChange" v-model="form.certificate_id" disabled clearable>
                ...
                <div class="status-wrap">
                    <div class="status">状态：(<span>{{form.cerstatus}}</span>)</div>
                    <div class="valid-time">有效期：{{form.starttime}}至{{form.endtime}}</div>
                </div>
            </el-form-item>
            <el-form-item label="证书编号" prop="certificate_num">
                <el-input v-model="form.certificate_num" disabled clearable></el-input>
            </el-form-item>
            ...
            <el-form-item class="commit-wrap" label="备注">
                <el-input
                        v-model="form.commit"
                        type="textarea"
                        maxlength="200"
...
import {getCertificateType, getCompanyNameList, studentObtainedSeeInfo, editorObtainedStudent} from '@/api/student'
import {timeout, isIdCard} from "@/api/util";
let tel_type_old = undefined
export default {
  name: "StudentEditorObtained",
  data() {
    ...
    return {
      ...
      rules: {
        ...
        company_name: [
          {required: true, message: '不能为空', trigger: 'change'},
          {min: 1, max: 20, message: '格式错误', trigger: 'blur'}
        ],
        ...
        certificate_num: [{required: true, message: '不能为空', trigger: 'blur'}]
      },
      validateStatus: {
        stu_name: true,
        stu_sex: true,
  ...
  methods: {
    goBack() {
      this.$router.push('/admin/stuobtained')
    },
    getInfo(id) {
      Promise.all([getCertificateType(), studentObtainedSeeInfo(id)]).then((res) => {
        let arr
        if (res[0].ret === 200) {
    ...
    submitFn() {
      ...
      this.$refs['formWrap'].validate((valid) => {
        if (valid) {
          ...
          editorObtainedStudent(this.form).then((res) => {
            if (res.ret === 200) {
