<template>
    <div class="create-unobtain">
        <top-header
                @triggerBackClick="goBack"
                :title="title"
                :backBtn="backBtn"></top-header>
        <el-form class="form"
                 :model="form"
                 :rules="rules"
            ...
            <el-form-item class="form-action">
                <el-button @click="submitFn" :disabled="submitBtnStatus || !debounceFlag" type="primary">立即新建</el-button>
...
import {getCompanyNameList, getCertificateType, createStudent, uploadImage} from '@/api/student'
import {timeout} from "@/api/util";
let tel_type_old = 1
export default {
  name: "StudentCreateUnobtain",
  data() {
    ...
    return {
      ...
      rules: {
        ...
        company_name: [
          {required: true, max: 20, message: '不能为空', trigger: 'change'},
          {required: true, max: 20, message: '不能为空', trigger: 'blur'},
        ],
